const moviesArr = [
  ...
  },
  {
    ...
    ticketPrice: 380,
    posterURL:
      "https://www.thefilmik.com/wp-content/uploads/2023/04/Fast-X.webp",
  },
  {
    title: "Attorney Woo",
    year: 2022,
...
const displayMovies = () => {
  ...
  for (let i = 0; i < moviesArr.length; i++) {
    ...
    movie.classList.add("movie", "card", "shadow-lg", "p-0", "border-0", "col");
...
<div class="card-body">
  <h4 class="card-title text-dark">${moviesArr[1].title}</h2>
  <p class="card-text text-danger"> <strong class="text-dark">Price:</strong>(${moviesArr[i].ticketPrice})</p>
  <button  class="btn btn-primary book-ticket">Book Ticket</button>
</div>
  `;
    moviesELe.append(movie);
  }
};
displayMovies();
...
const modalDisplay = () => {
  const bookTcks = document.querySelectorAll(".book-ticket");
  const windowModal = document.querySelector(".window-modal");

  for (let i = 0; i < bookTcks.length; i++) {
    bookTcks[i].addEventListener("click", function () {
      for (let j = 0; j < moviesArr.length; j++) {
        if (i === j) {
          ...
          <span class="close-btn fs-6 fw-bold  text-danger">X</span>
          <h2 class="fw-bold">${title} (${year})</h2>
          <h4><strong>price:</strong>($${price})</h4>
          <h4><strong>VAT:</strong>(${VAT}%)</h4>
          <h3><strong>Subtotal:</strong>($${grandTotal})</h3>
          <button class="btn btn-success fw-bold mt-5">Buy now</button>
          `;
